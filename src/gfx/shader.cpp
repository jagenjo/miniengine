...
const char* texture_pixel_shader = "\
	//global info about the light \
...
Shader* Shader::getGenericTextureShader()
{
	if (texture_shader) return texture_shader;
	Shader* sh = new Shader();
	sh->compileFromMemory(texture_vertex_shader,texture_pixel_shader);
	texture_shader = sh;
